...
impl Instructions {
    fn new(ttd: &str, tte: &str, tts: &str, must_eat: Option<&String>) -> Result<Self, String> {
        let ttd = string_to(ttd, "error parsing time to die")?;
        let tte = string_to(tte, "error parsing time to sleep")?;
        let tts = string_to(tts, "error parsing time to eat")?;

        let must_eat = match must_eat {
...
fn main() {
    ...
    let instructions = match Instructions::new(&args[2], &args[3], &args[4], args.get(5)) {
        ...
        thread::spawn(move || {
            ...
            thread::spawn(move || loop {
                ...
                if instructions.must_eat.is_some()
                    && *must_eat_rec_checker.lock().unwrap() == nums_of_philos
                {
